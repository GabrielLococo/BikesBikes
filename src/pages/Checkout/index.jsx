    ...
    return (<>
        {!contextInfo.orderId ? <main className="checkoutContainer">
            ...
            <form onSubmit={handleConfirm}>
                ...
                <Button type='submit' disabled={!(name !== '' && phone !== '' && email !== '') || (contextInfo.cartProducts.length < 1)} style={{ margin: '0 auto' }} variant='success'>Finalizar la compra</Button>
            </form>
            <h2>Precio total : US$ {totalPrice}</h2>
        </main>
            : <main className="checkoutContainer">
                <h1>😍Gracias por tu compra!😍</h1>
                <h2>ID de tu compra: </h2> 
                <h3 style={{background:'grey', width:'100%', textAlign:'center',borderRadius:'10px',color:'black',padding:'10px'}} > {contextInfo.orderId}</h3>
                <Link to={'/'}><Button onClick={goBack} variant='success'>Volver</Button></Link></main>}</>
    );
}
